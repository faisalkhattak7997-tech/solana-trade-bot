...
if KEY:
    log(f"Key loaded: {len(KEY)} chars, starts with {KEY[:4]}")
else:
    log("WARNING: No private key set!")
...
def get_balance():
    try:
        ...
        if "result" in data and "value" in data["result"]:
            return round(data["result"]["value"]/1e9, 6)
        log(f"Balance response: {data}")
        return None
...
def main():
    ...
    # Balance - always fetch fresh from Helius
    fetched = get_balance()
    cached = float(d.get("wallet",{}).get("balance",0) or 0)
    if fetched is not None and fetched >= 0:
        bal = fetched
        log(f"Balance: {bal} SOL (live from Helius)")
    elif cached > 0:
        bal = cached
        log(f"Balance: {bal} SOL (cached)")
    else:
        # Last resort - check via public RPC
        try:
            r2 = requests.post("https://api.mainnet-beta.solana.com",
                json={"jsonrpc":"2.0","id":1,"method":"getBalance","params":[WALLET]},
                headers={"Content-Type":"application/json"}, timeout=15)
            bal = round(r2.json()["result"]["value"]/1e9, 6)
            log(f"Balance: {bal} SOL (public RPC)")
        except:
            bal = 0.0
            log("Balance: could not fetch")
